...
class A {
	public:
		...
		void executeProcess(int *burstTime[],int *noOfProcesses,int *arrivalTime[], int *processID[],int *queue[]){
...
int main(){
	...
	int total = noOfProcesses;
	A obj;
	obj.executeProcess(&burstTime, &noOfProcesses, &arrivalTime, &processID, &queuew);
}
